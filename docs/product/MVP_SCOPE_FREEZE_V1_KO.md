...
> ‘같이 시작하기’ 무료 텍스트 샘플 1개와 로컬 생활 행동 기록을 구현한다.
> 아래 기존 집중 시간 preset은 집중 세션에만 적용한다. 생활 안내 2분은 제한시간이 아니다.
> 결제·성장 규칙·연령 게이트 변경 및 출시는 이번 승인 범위에 포함하지 않는다.
> 전체 요구사항, 기존 가격 계약과의 차이, 후속 검증은
> [같이 시작하기 단계별 계획](COMPANION_IMPLEMENTATION_PLAN_KO.md)을 따른다.
...
| 2026-09-10 | v1.3 | 홈 → 무료 대표 샘플 1개 → 명시적 결과 → 별도 로컬 기록 및 테스트. 운영 공개·결제·성장 변경 제외 | 이번 사용자 작업 지시(개발 범위만) |
